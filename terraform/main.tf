...
resource "aws_ecr_repository" "app_repo" {
  name                 = var.ecr_repository # Assuming this variable exists or is defined in variables.tf
  ...
  container_definitions = jsonencode([
    {
      ...
      image       = "${aws_ecr_repository.app_repo.repository_url}:latest" # Placeholder for the latest tag
      cpu         = 256
      memory      = 512
      essential   = true
      portMappings = [
  ...
  dashboard_body = jsonencode({
    widgets = [
      ...
      },
      {
        ...
        properties = {
          metrics = [
            ["AWS/ApplicationELB", "RequestCount", "LoadBalancer", aws_lb.main.name, "TargetGroup", aws_lb_target_group.app.name],
          ]
          period = 300
          stat   = "Sum"
          region = var.aws_region
